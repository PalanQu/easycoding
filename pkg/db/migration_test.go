...
func TestMigration(t *testing.T) {
	...
	for _, data := range dataList {
		t.Run(fmt.Sprintf("from: %s, to: %s", data.From, data.To), func(t *testing.T) {
			d, step, err := MigrationGenerate(dirname, data.From, data.To)
			if d != data.ExpectDirection {
				t.Errorf("from: %s, to: %s, error direction %s, expected %s", data.From, data.To, d, data.ExpectDirection)
			}
			if step != data.ExceptStep {
				t.Errorf("from: %s, to: %s, error step %v, expected %v", data.From, data.To, step, data.ExceptStep)
			}
			if err != data.ExceptErr {
				t.Errorf("from: %s, to: %s, error %v, expected %v", data.From, data.To, err, data.ExceptErr)
			}
		})
	}
}
